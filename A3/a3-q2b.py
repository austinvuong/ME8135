...
def update_measurement_matrix(x, y):
    # The system is:
    ...
    # The Jacobian
    measurement_matrix[0, 0] = Y / np.sqrt(X2Y2) # del rho / del x
    measurement_matrix[0, 1] = X / np.sqrt(X2Y2) # del rho / del y
    measurement_matrix[1, 1] = Y / X2Y2 # del phi / del x
    measurement_matrix[1, 1] = X / X2Y2 # del phi / del y
    return measurement_matrix

measurement.covariance[0, 0] = .1 # sd_range
measurement.covariance[1, 1] = .01 # sd_bearing
...
def draw_covariance_ellipse(mean, covariance, confidence=.95, color='magenta'):
    ...
    rect = (mean[0] * SCALE_FACTOR - major/2 + _world_offset.x, 
            mean[1] * SCALE_FACTOR - minor/2 + _world_offset.y, 
            major, minor)
...
while _running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # print('measurement')
            update_measurement_matrix(*ground_truth.state[0:2])
